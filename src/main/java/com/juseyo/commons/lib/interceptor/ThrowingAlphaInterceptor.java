/**
 * @file			ThrowingAlphaInterceptor.java
 * @brief   	1팀용 인터셉터 ( 에러처리 전용 ) 
 * @author  개발1팀 / 파트5
...
import com.juseyo.commons.lib.interceptor.exception.MessageException;
...
/**
* @brief 	ThrowingAlphaInterceptor
* @author 	개발1팀 / 파트5
* @version 1.0
* @date    생성: 2014-03-14
* @date    최종수정: 2014-03-14
...
*/
@Aspect
public class ThrowingAlphaInterceptor {
	 ...
	 * @details	
	 * @param 	
	 * @return		com.wemakeprice.performancereview.controller.AlphaReviewController
	 */
	@Pointcut("execution(* com.wemakeprice.performancereview..*(..)) || execution(* com.wemakeprice.visitor..*Controller*.*(..)) ")
	public void pointcut4throwingLog() {}
	
	
	 /**
	 * @brief		모든 호출에 대한 exception 처리 ( 1팀용 )
	 * @details	
	 * @param 	joinPoint
	 * @throws Exception
	 * @return		
	...
	public void _throwing(JoinPoint joinPoint, Throwable error) throws Exception {
		...
		if(error instanceof NullDataException) {
            ...
            LogUtil.infoThrow(_logger_error, joinPoint.getTarget().getClass().getName(), joinPoint.getSignature().getName(), error);
		} else if (error instanceof MessageException){ // 메시지로 에러내용을 보냅니다.
            MessageException msg = (MessageException)error;
            // 로그인 에러
            map = messageSetter.setCommonMessage( msg.getMessageId(), this.getClass().getName());
            LogUtil.infoThrow(_logger_error, joinPoint.getTarget().getClass().getName(), joinPoint.getSignature().getName(), error);
        } else if (error instanceof ModelAndViewDefiningException){
            throw (ModelAndViewDefiningException)error;
        } else {
